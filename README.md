...
> AI-Code-Security-Auditor - Open source tool by AetherCodeHQ

`ai` `cli` `golang` `open-source` `sast` `security`

## What is it?

**Ai Code Security Auditor** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/AI-Code-Security-Auditor.git
...
```bash
# Basic usage
./AI-Code-Security-Auditor --path ./target

# With options
./AI-Code-Security-Auditor --path ./target --format json --output report.json

# Verbose mode
./AI-Code-Security-Auditor --path ./target --verbose
```
...
### Basic scan
```bash
./AI-Code-Security-Auditor --path ./my-project
```

### JSON report
```bash
./AI-Code-Security-Auditor --path ./my-project --format json --output report.json
```
...
- name: Run Ai Code Security Auditor
  run: ./AI-Code-Security-Auditor --path . --format json --output report.json
```
